...
- Konten siap = pre-generated (takeout): lesson/quiz/exam TIDAK lagi di-generate AI dari halaman user — cache-only (`getLessonAction`/`getQuizAction`/`getExamAction` return error "belum tersedia" jika kosong); `prefetchLessonAction` dihapus. **General practice TETAP AI on-demand** (`getGeneralPracticeAction` — 100% fresh, TANPA cache: generate baru setiap request dengan tema acak via `GENERAL_PRACTICE_THEMES` + `buildGeneralPracticePrompt(language, level, theme)`; soal menantang di ambang atas level). Weakness practice, chat, story, pronunciation, placement juga tetap AI on-demand
- Bulk pre-generation admin (panel "Konten"): pilih BAHASA → generate SEMUA level (default: bagian 1-3, modifier normal+hard+easy, 5 varian quiz + exam); `buildContentWorkList` (pure, `lib/admin.ts`) + `resolveLanguageContentStatus`/`generateContentChunkAction`/`getContentGenerationStatusAction` (`lib/actions/admin.ts`) — client-driven loop 1 unit/panggilan, idempotent (skip cache existing); general_practice TIDAK di work list (AI fresh per request)
