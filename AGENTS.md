...
- Bulk pre-generation admin (panel "Konten"): `buildContentWorkList` (pure, `lib/admin.ts`) + `generateContentChunkAction`/`getContentGenerationStatusAction` (`lib/actions/admin.ts`) — client-driven loop 1 unit/panggilan, idempotent (skip cache existing), work list = lesson per (goal, part, modifier) + quiz 5 varian per goal + exam + general_practice
