...
- Bulk pre-generation admin (panel "Konten"): pilih BAHASA → generate SEMUA level (default: bagian 1-3, modifier normal+hard+easy, 5 varian quiz per goal + 5 exam + 15 pool general_practice); `buildContentWorkList` (pure, `lib/admin.ts`) + `resolveLanguageContentStatus`/`generateContentChunkAction`/`getContentGenerationStatusAction` (`lib/actions/admin.ts`) — client-driven loop 1 unit/panggilan, idempotent (skip cache existing); target readiness di `computeLevelContentTargets` pakai `CONTENT_EXAM_VARIANTS`/`CONTENT_GENERAL_PRACTICE_VARIANTS` (satu sumber kebenaran di `lib/admin.ts`)
