...
- Bulk pre-generation admin (panel "Konten"): pilih BAHASA → generate SEMUA level (default: bagian 1-3, modifier normal+hard+easy, 5 varian quiz per goal + 5 exam + 15 pool general_practice); `buildContentWorkList` + engine `resolveLanguageContentStatus`/`findNextUndoneUnit`/`generateOneContentUnit`/`processContentBatch` (pure/berbagi, `lib/admin.ts`) + action `generateContentChunkAction` (manual loop, tab terbuka) + **background**: `app/api/content-generation/route.ts` (Vercel Background Functions `after()` — tiap invokasi proses batch ≤4,5 mnt lalu self-chain via POST resume Bearer `CRON_SECRET` sampai selesai; tab boleh ditutup; job di `content_generation_jobs` — status running/done/failed/cancelled, stale >8 mnt auto-failed saat start ulang); cancel via `cancelContentGenerationAction`. Unit yang gagal AI ≥3x di-skip (tabel `failed_content_units`, anti-deadlock) — bahasa baru siap hanya jika SEMUA unit (termasuk yang pernah gagal) berhasil digenerate; reset via `resetFailedContentUnitsAction` (tombol panel); `parseAiJson` punya repair JSON terpotong
