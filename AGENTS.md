...
- Bulk pre-generation admin (panel "Konten" — **per-unit, bukan bulk**): pilih BAHASA → LEVEL → TIPE (lesson bagian 1-3 × modifier, quiz/exam/general_practice 1 varian) → `generateSpecificUnitAction` (1 konten/klik, cap varian: quiz 5/goal, exam 5, gp 15; lesson duplikat ditolak). Default target level: bagian 1-3, modifier normal+hard+easy, 5 varian quiz per goal + 5 exam + 15 pool general_practice; `buildContentWorkList` + engine `resolveLanguageContentStatus`/`findNextUndoneUnits`/`findNextUndoneUnit`/`generateOneContentUnit` (pure/berbagi, `lib/admin.ts`). **Generate massal via CLI**: `npm run content:generate <Bahasa>` (`scripts/generate-content.ts` — 3 unit paralel, progress realtime, resume idempotent, tunggu cooldown unit gagal otomatis). Unit yang gagal AI ≥3x di-skip SEMENTARA (`failed_content_units`, cooldown 30 mnt via `FAILED_SKIP_THRESHOLD`/`FAILED_COOLDOWN_MS`) — bahasa baru siap hanya jika SEMUA unit berhasil digenerate; reset via `resetFailedContentUnitsAction` (tombol panel); `parseAiJson` punya repair JSON terpotong
