...
- Konten siap = pre-generated (takeout): lesson/quiz/general-practice/exam TIDAK lagi di-generate AI dari halaman user — cache-only (`getLessonAction`/`getQuizAction`/`getGeneralPracticeAction`/`getExamAction` return error "belum tersedia" jika kosong); `prefetchLessonAction` dihapus. Weakness practice, chat, story, pronunciation, placement tetap AI on-demand
