...
- Bulk pre-generation admin (panel "Konten" — **per-unit, bukan bulk**): pilih BAHASA → LEVEL → TIPE (lesson bagian 1-3 × modifier, quiz/exam/general_practice 1 varian) → `generateSpecificUnitAction` (1 konten/klik — **tanpa batas varian quiz**; lesson duplikat ditolak). Target minimal per level = 1 varian per unit (`CONTENT_QUIZ_VARIANTS`/`CONTENT_EXAM_VARIANTS`/`CONTENT_GENERAL_PRACTICE_VARIANTS` = 1): lesson (goal, bagian 1-3, modifier) + 1 quiz per goal + 1 exam + 1 general_practice; `buildContentWorkList` + engine `resolveLanguageContentStatus`/`findNextUndoneUnits`/`findNextUndoneUnit`/`generateOneContentUnit` (pure/berbagi, `lib/admin.ts`). **Generate massal via CLI**: `npm run content:generate <Bahasa>` (`scripts/generate-content.ts` — 3 unit paralel, progress realtime, resume idempotent, tunggu cooldown unit gagal otomatis). Unit yang gagal AI ≥3x di-skip SEMENTARA (`failed_content_units`, cooldown 30 mnt via `FAILED_SKIP_THRESHOLD`/`FAILED_COOLDOWN_MS`) — bahasa baru siap hanya jika SEMUA unit punya minimal 1 konten; reset via `resetFailedContentUnitsAction` (tombol panel); `parseAiJson` punya repair JSON terpotong
