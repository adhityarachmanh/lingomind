...
- Konten siap = pre-generated (takeout): lesson/quiz/exam TIDAK lagi di-generate AI dari halaman user — cache-only (`getLessonAction`/`getQuizAction`/`getExamAction` return error "belum tersedia" jika kosong); `prefetchLessonAction` dihapus. **General practice TETAP AI on-demand** (`getGeneralPracticeAction` — cache dulu, generate bila kosong; soal menantang di ambang atas level via `buildGeneralPracticePrompt`). Weakness practice, chat, story, pronunciation, placement juga tetap AI on-demand
- Bulk pre-generation admin (panel "Konten"): pilih BAHASA → generate SEMUA level (default: bagian 1-3, modifier normal+hard+easy, 5 varian quiz + exam + general_practice); `buildContentWorkList` (pure, `lib/admin.ts`) + `resolveLanguageContentStatus`/`generateContentChunkAction`/`getContentGenerationStatusAction` (`lib/actions/admin.ts`) — client-driven loop 1 unit/panggilan, idempotent (skip cache existing)
