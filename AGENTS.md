...
- `app/api/cron/daily-reminder` — Vercel Cron `0 1 * * *` UTC (= 08:00 WIB): kirim reminder harian ke user aktif via `lib/reminder.ts`; guard `x-vercel-cron` ATAU Bearer `CRON_SECRET`
