...
describe("computeLevelContentTargets", () => {
  it("lesson = goals*9, quiz = goals*1+2 (1 varian per unit: quiz, exam, general_practice)", () => {
    expect(computeLevelContentTargets(3)).toEqual({ lessonTotal: 27, quizTotal: 5 });
    expect(computeLevelContentTargets(0)).toEqual({ lessonTotal: 0, quizTotal: 2 });
    expect(computeLevelContentTargets(10)).toEqual({ lessonTotal: 90, quizTotal: 12 });
  });
});

describe("isLevelReady", () => {
  it("benar saat lesson & quiz cache memenuhi target", () => {
    expect(isLevelReady(27, 5, 3)).toBe(true);
    expect(isLevelReady(27, 6, 3)).toBe(true);
  });
  it("salah saat salah satu belum memenuhi target", () => {
    expect(isLevelReady(26, 5, 3)).toBe(false);
    expect(isLevelReady(27, 4, 3)).toBe(false);
    expect(isLevelReady(27, 2, 3)).toBe(false);
...
describe("isLanguageReady", () => {
  it("benar hanya jika SEMUA level lengkap", () => {
    expect(isLanguageReady([{ goalCount: 3, lessonCount: 27, quizCount: 5 }])).toBe(true);
    expect(
      isLanguageReady([
        { goalCount: 3, lessonCount: 27, quizCount: 5 },
        { goalCount: 2, lessonCount: 18, quizCount: 4 },
      ])
    ).toBe(true);
  });
