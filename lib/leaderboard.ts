...
export async function getWeeklyLeague(
  ...
  const existing = await db.userLeagueMember.findFirst({
    where: { email, group: { weekStartDate: weekStart } },
    include: { group: true },
  });
  let groupId: number;
  let division: string;
    ...
    try {
      const assigned = await db.$transaction(async (tx) => {
        ...
        if (prev.length > 0) {
          ...
          nextDivisionIdx = decideNextDivision(prevIdx >= 0 ? prevIdx : null, Number(prev[0].rnk));
        }
      ...
      const reread = await db.userLeagueMember.findFirst({
        where: { email, group: { weekStartDate: weekStart } },
        include: { group: true },
      });
      if (!reread) throw e;
      groupId = reread.groupId;
      division = reread.group.division;
    }
  }
