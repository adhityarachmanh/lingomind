...
#[cfg(feature = "server")]
static LANGUAGES_CACHE: OnceCell<Vec<LanguageCourse>> = OnceCell::const_new();

#[cfg(feature = "server")]
static CURRICULUM_CACHE: OnceCell<Vec<CurriculumLevel>> = OnceCell::const_new();

#[server]
pub async fn get_all_languages() -> Result<Vec<LanguageCourse>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        let langs = LANGUAGES_CACHE.get_or_try_init(|| async {
            ...
            Ok::<Vec<LanguageCourse>, ServerFnError>(languages)
        }).await?;
...
pub async fn get_all_curriculum() -> Result<Vec<CurriculumLevel>, ServerFnError> {
    #[cfg(feature = "server")]
    {
        let curriculum = CURRICULUM_CACHE.get_or_try_init(|| async {
            let database_url = env::var("DATABASE_URL").map_err(|_| ServerFnError::new("DATABASE_URL not set"))?;
            let pool = PgPool::connect(&database_url).await.map_err(|e| ServerFnError::new(e.to_string()))?;
            ...
            Ok::<Vec<CurriculumLevel>, ServerFnError>(curriculum)
        }).await?;
