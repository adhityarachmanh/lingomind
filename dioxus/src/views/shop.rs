...
use crate::services::shop::{get_shop_summary_server, buy_shop_item_server};
...
pub fn Shop() -> Element {
    ...
    if user_opt.is_none() {
        return rsx! {
            div { class: "min-h-screen bg-slate-50 flex justify-center items-center p-8",
                "Harap login."
            }
        };
    }
    ...
    let email_for_summary = email.clone();
    let mut summary_resource = use_resource(move || {
        let u = email_for_summary.clone();
        async move { get_shop_summary_server(u).await }
    });

    let summary = summary_resource.value()().and_then(|r| r.ok());
    let items = summary.as_ref().map(|s| s.items.clone()).unwrap_or_default();
    let coins = summary.as_ref().and_then(|s| s.engagement.as_ref()).map(|e| e.coins).unwrap_or(0);
            ...
            div { class: "max-w-4xl mx-auto space-y-6",
                ...
                div { class: "relative overflow-hidden bg-gradient-to-r from-amber-500 to-orange-600 rounded-3xl p-6 sm:p-10 shadow-xl text-white",
                    div { class: "relative z-10 flex flex-col sm:flex-row justify-between items-center gap-4",
                        div {
                            ...
                            p { class: "text-amber-50 opacity-90 font-medium",
                                "Gunakan koinmu untuk membeli item eksklusif!"
                            }
                        }
                        div { class: "bg-white/20 backdrop-blur-md rounded-2xl p-4 border border-white/20 text-center flex flex-col items-center min-w-[120px]",
                            p { class: "text-xs font-bold uppercase tracking-wider text-amber-100",
                                "Saldo Koin"
                            }
                ...
                if !buy_status().is_empty() {
                    div { class: "bg-teal-50 dark:bg-teal-900/30 border border-teal-200 dark:border-teal-800 text-teal-800 dark:text-teal-300 p-4 rounded-xl flex items-center gap-3 animate-fade-in shadow-sm",
                        ...
                        button {
                            class: "ml-auto text-teal-500 hover:text-teal-700",
                            onclick: move |_| buy_status.set(String::new()),
                            "✕"
                        }
                    }
                }
                ...
                // Grid Items - Utilitas
                div {
                    h2 { class: "text-2xl font-black text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2",
                        "🚑 Utilitas & Penyelamat Nyawa"
                    }
                    div { class: "grid grid-cols-1 sm:grid-cols-2 gap-4",
                        for item in items
                            .iter()
                            .filter(|i| {
                                !i.effect_type.starts_with("profile_frame_")
                                    && !i.effect_type.starts_with("title_")
                            ...
                            div {
                                ...
                                div { class: "mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between",
                                    div { class: "flex items-center gap-1.5",
                                        span { class: "text-lg", "🪙" }
                                        span { class: "font-black text-lg text-amber-600 dark:text-amber-500",
                                            "{item.cost}"
                                        }
                                    }
                                    {
                                        ...
                                        rsx! {
                                            button {
                                                class: if item.is_owned { "px-5 py-2.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 font-bold rounded-xl text-sm cursor-not-allowed" } else if can_afford { "px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm transition-all shadow-sm shadow-indigo-500/30 disabled:opacity-50" } else { "px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-400 font-bold rounded-xl text-sm cursor-not-allowed" },
                ...
                // Grid Items - Kosmetik
                div {
                    h2 { class: "text-2xl font-black text-slate-800 dark:text-slate-100 mb-4 mt-8 flex items-center gap-2",
                        "🏆 Status & Gengsi (Kosmetik)"
                    }
                    div { class: "grid grid-cols-1 sm:grid-cols-2 gap-4",
                        for item in items
                            .iter()
