            ...
            if let Some(cached) = crate::services::offline::get_offline_flashcards(&selected_language()) {
                ...
                return rsx! {
                    div { class: "p-6 text-rose-600 dark:text-rose-400 font-sans",
                        "Gagal memuat flashcard: {e} (Tidak ada cache offline)"
                    }
                }
            }
        }
        ...
        return rsx! {
            div { class: "min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 flex items-center justify-center p-6 font-sans",
                div { class: "bg-white dark:bg-slate-900 border border-slate-200/20 dark:border-slate-700 rounded-3xl p-8 max-w-md w-full text-center shadow-xl hover:shadow-2xl transition-all",
                    ...
                    h2 { class: "text-2xl font-black text-slate-800 dark:text-slate-200 mb-2 tracking-tight",
                        "Semua Kartu Bersih!"
                    }
        ...
        return rsx! {
            div { class: "min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 flex items-center justify-center p-6 font-sans",
                div { class: "bg-white dark:bg-slate-900 border border-slate-200/20 dark:border-slate-700 rounded-3xl p-8 max-w-md w-full text-center shadow-xl hover:shadow-2xl transition-all",
                    ...
                    p { class: "text-slate-500 dark:text-slate-400 text-sm mb-6 font-semibold leading-relaxed",
                        "Hebat! Semua kartu di sesi ini telah selesai diulas secara optimal."
                    }
                    Link {
                        to: Route::Dashboard {},
                        class: "inline-block bg-teal-600 hover:bg-teal-700 text-white font-bold px-8 py-3 rounded-2xl transition-all shadow-md hover:shadow-lg hover:shadow-teal-600/20",
            ...
            div { class: "max-w-xl w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-xl relative",
                ...
                div { class: "flex justify-between items-center mb-6",
                    ...
                    span { class: "flex gap-2",
                        ...
                        if is_offline {
                            span { class: "text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-50/30 dark:bg-rose-900/30 px-3 py-1 rounded-full border border-rose-100 dark:border-rose-900/50",
                                "📵 Offline Mode"
                            }
                        }
                    }
                ...
                div { class: "bg-white dark:bg-slate-900 border-2 border-slate-100/50 dark:border-slate-800 rounded-2xl p-6 sm:p-8 mb-8 text-center shadow-md relative min-h-[220px] flex flex-col justify-center transition-all hover:border-teal-100/50 dark:border-teal-900/50",
                    ...
                    p { class: "text-[10px] font-extrabold text-slate-400 mb-2 uppercase tracking-widest",
                        "KATA / FRASA"
                    }
                    p { class: "text-3xl font-black text-slate-800 dark:text-slate-200 leading-snug tracking-tight mb-2",
                        "{current.front_text}"
                    }
                    ...
                    if show_back() {
                        div { class: "mt-6 pt-6 border-t-2 border-slate-50 animate-fade-in",
                            ...
                            p { class: "text-2xl font-extrabold text-teal-600 dark:text-teal-400 tracking-tight",
                                "{current.back_text}"
                            }
                        }
                    }
                ...
                if !show_back() {
                    ...
                    div { class: "grid grid-cols-3 gap-3",
                        button {
                            ...
                            onclick: move |_| {
                                spawn(async move {
                                    let _ = review_flashcard_server(card_id_again, 2).await;
                                });
                                show_back.set(false);
                                let next = index() + 1;
                                if next >= total_cards {
                            ...
                            span { class: "text-lg", "🔴" }
                            span { "Ulangi" }
                            span { class: "text-[9px] font-semibold text-rose-500/80 mt-0.5",
                                "Lupa / Sulit"
                            }
                        }
                        button {
                            ...
                            onclick: move |_| {
                                spawn(async move {
                                    let _ = review_flashcard_server(card_id_good, 4).await;
                                });
                                show_back.set(false);
                                let next = index() + 1;
                                if next >= total_cards {
                            ...
                            span { class: "text-lg", "🟡" }
                            span { "Bagus" }
                            span { class: "text-[9px] font-semibold text-amber-500/80 mt-0.5",
                                "Cukup Ingat"
                            }
                        }
                        button {
                            ...
                            onclick: move |_| {
                                spawn(async move {
                                    let _ = review_flashcard_server(card_id_easy, 5).await;
                                });
                                show_back.set(false);
                                let next = index() + 1;
                                if next >= total_cards {
                            ...
                            span { class: "text-lg", "🟢" }
                            span { "Mudah" }
                            span { class: "text-[9px] font-semibold text-teal-500/80 mt-0.5",
                                "Sangat Ingat"
                            }
                        }
                    }
