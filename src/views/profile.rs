use dioxus::prelude::*;
use crate::services::profile::{get_profile_summary_server, equip_frame_server, equip_title_server, equip_color_server};
...
pub fn Profile(email: String) -> Element {
    ...
    let requesting_email = me_opt.as_ref().map(|me| me.email.clone()).unwrap_or_default();
    
    let mut summary_resource = use_resource(move || {
        let e = email_clone.clone();
        let r = requesting_email.clone();
        async move { get_profile_summary_server(e, r).await }
    });

    let content = match summary_resource.value()() {
        Some(Ok(summary)) => {
            ...
            all_frames.extend(summary.frames.clone());
            let mut all_titles = vec!["default".to_string()];
            all_titles.extend(summary.titles.clone());
            let mut all_colors = vec!["default".to_string()];
            all_colors.extend(summary.colors.clone());
