        ...
        div { class: "min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 flex flex-col justify-center items-center p-6",
            div { class: "bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl max-w-md w-full border border-slate-200 dark:border-slate-700 text-center",
                ...
                if is_loading() {
                    ...
                    h2 { class: "text-2xl font-extrabold text-slate-700 dark:text-slate-300 mb-2", "Memproses..." }
                    p { class: "text-slate-500 dark:text-slate-400 font-medium text-sm mb-6", "{verification_status}" }
                } else if is_success() {
                    h2 { class: "text-2xl font-extrabold text-teal-600 dark:text-teal-400 mb-2", "Verifikasi Berhasil!" }
                    p { class: "text-slate-500 dark:text-slate-400 font-medium text-sm mb-8", "{verification_status}" }
                    ...
                    h2 { class: "text-2xl font-extrabold text-rose-600 dark:text-rose-400 mb-2", "Verifikasi Gagal" }
                    p { class: "text-slate-500 dark:text-slate-400 font-medium text-sm mb-8", "{verification_status}" }
