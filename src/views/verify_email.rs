        ...
        div { class: "min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-center items-center p-6",
            div { class: "bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-slate-200 text-center",
                ...
                img {
                    ...
                    alt: "LingoMind Logo",
                    class: "w-20 h-20 rounded-3xl mx-auto mb-6 shadow-md object-cover border border-slate-100",
                }

                if is_loading() {
                    div { class: "animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-teal-500 mx-auto mb-4" }
                    h2 { class: "text-2xl font-extrabold text-slate-700 mb-2", "Memproses..." }
                    p { class: "text-slate-500 font-medium text-sm mb-6", "{verification_status}" }
                } else if is_success() {
                    h2 { class: "text-2xl font-extrabold text-teal-600 mb-2", "Verifikasi Berhasil!" }
                    p { class: "text-slate-500 font-medium text-sm mb-8", "{verification_status}" }
                    ...
                    h2 { class: "text-2xl font-extrabold text-rose-600 mb-2", "Verifikasi Gagal" }
                    p { class: "text-slate-500 font-medium text-sm mb-8", "{verification_status}" }
