    ...
    rsx! {
        div { class: "min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-center items-center p-6 font-sans",
            div { class: "bg-white p-8 rounded-2xl shadow-lg max-w-md w-full border border-slate-200 text-center",
                ...
                h2 { class: "text-3xl font-extrabold text-teal-600 mb-2", "Reset Password" }
                p { class: "text-slate-500 text-sm mb-6 font-medium", "Masukkan kata sandi baru Anda di bawah ini." }
                ...
                if !reset_success() {
                    ...
                    div { class: "mb-4 text-left",
                        label { class: "block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2", "Kata Sandi Baru" }
                    ...
                    div { class: "mb-6 text-left",
                        label { class: "block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2", "Konfirmasi Kata Sandi" }
                        input {
                            r#type: if show_passwords() { "text" } else { "password" },
