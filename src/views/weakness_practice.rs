...
fn speak_with_edge_or_fallback(edge_tts_voice: String, fallback_lang_code: String, text: String, speed: f32) {
    ...
    spawn(async move {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            let _ = (&fallback_lang_code, &normalized_text, speed);
        }
    });
}
...
pub fn WeaknessPractice(goal: String) -> Element {
    ...
    let Some(quiz_data) = quiz_res.value()() else {
        return rsx! {
            div { class: "min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center items-center p-6 font-sans text-center relative overflow-hidden",
                div { class: "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-rose-500/10 dark:bg-rose-500/5 rounded-full blur-3xl animate-pulse pointer-events-none" }
                
                div { class: "relative w-28 h-28 mb-8",
                    div { class: "absolute inset-0 rounded-full border-4 border-dashed border-rose-500/30 dark:border-rose-400/20 animate-[spin_10s_linear_infinite]" }
                    div { class: "absolute inset-2 rounded-full border-t-4 border-l-4 border-rose-500 dark:border-rose-400 animate-spin" }
                ...
                div { class: "bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/50 rounded-2xl p-5 shadow-sm max-w-sm w-full z-10",
                    div { class: "flex items-start gap-4",
                        div { class: "bg-rose-50 dark:bg-rose-900/30 p-2.5 rounded-xl text-rose-600 dark:text-rose-400 shrink-0", 
                            "✨" 
                        }
                        div { class: "text-left",
                            ...
                            p { class: "text-sm text-slate-600 dark:text-slate-300 font-medium leading-relaxed", 
                                "Sedang menganalisis riwayat kelemahan Anda dan menyusun soal latihan yang tepat." 
                            }
                        }
                    }
                }
    ...
    let quiz = match quiz_data {
        Ok(q) => q,
        Err(e) => return rsx! {
            div { class: "p-6 text-rose-600 dark:text-rose-400 font-bold",
                "Gagal generate practice quiz: {e}"
            }
        },
    };
            ...
            div { class: "min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 flex flex-col justify-center items-center px-4 py-6 sm:p-8 font-sans",
                div { class: "bg-white dark:bg-slate-900 p-6 sm:p-10 rounded-3xl border border-slate-200/30 dark:border-slate-700 text-center max-w-md w-full shadow-xl",
                    ...
                    p { class: "text-slate-500 dark:text-slate-400 text-sm mb-8 font-medium",
                        "Kamu berhasil menuntaskan latihan fokus kelemahan."
                    }
    ...
    if quiz.questions.is_empty() {
        return rsx! {
            div { class: "p-8 text-amber-600 dark:text-amber-400 font-bold text-center",
                "Practice quiz kosong. Coba refresh halaman."
            }
        };
    }
                ...
                div {
                    ...
                    div { class: "flex items-center gap-4 mb-6 sm:mb-8 border-b border-slate-100/50 dark:border-slate-800 pb-4",
                        ...
                        span { class: "text-xs font-bold text-slate-500 dark:text-slate-400 font-mono shrink-0",
                            "{idx() + 1}/{quiz.questions.len()}"
                        }
                    }

                    // Level and status badges
                    div { class: "flex flex-wrap items-center gap-2 mb-4",
                        span { class: "text-[10px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50/30 dark:bg-amber-900/30 px-2.5 py-1 rounded-full uppercase tracking-wider border border-amber-100",
                            "Fokus Kelemahan: {weakness_topic}"
                        }
