...
pub fn WeaknessAnalytics() -> Element {
    ...
    rsx! {
        div { class: "min-h-screen bg-slate-50 text-slate-900 p-4 sm:p-8 flex items-center justify-center font-sans",
            div { class: "max-w-4xl w-full bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xl",
                ...
                div { class: "flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8",
                    ...
                    Link { 
                        to: Route::Dashboard {}, 
                        class: "text-sm bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold px-5 py-2.5 rounded-2xl transition-all shadow-sm text-center", 
                        "Kembali ke Dashboard" 
                    }
                }

                // Interactive Premium Tabs
                div { class: "flex bg-slate-100/80 p-1.5 rounded-2xl mb-8 border border-slate-200/50 max-w-md",
                    button {
                        class: format!(
                            "flex-1 py-2.5 rounded-xl font-bold text-sm transition-all duration-200 {}",
                            if active_tab() == 0 { "bg-white text-slate-800 shadow-md" } else { "text-slate-500 hover:text-slate-800" }
                        ),
                    ...
                    button {
                        class: format!(
                            "flex-1 py-2.5 rounded-xl font-bold text-sm transition-all duration-200 {}",
                            if active_tab() == 1 { "bg-white text-slate-800 shadow-md" } else { "text-slate-500 hover:text-slate-800" }
                        ),
                ...
                if active_tab() == 0 {
                    ...
                    if items.is_empty() {
                        div { class: "bg-slate-50 border border-slate-200 border-dashed rounded-2xl p-12 text-center",
                            ...
                            p { class: "text-slate-500 font-bold text-base", "Belum ada data kelemahan untuk bahasa ini." }
                            ...
                            for item in items {
                                {
                                    ...
                                    rsx! {
                                        div { class: "bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between hover:border-teal-100",
                                            div { class: "mb-4",
                                                p { class: "text-base text-slate-800 font-black mb-2 group-hover:text-teal-600 transition-colors", "{item.topic}" }
                                            ...
                                            div { class: "space-y-4 pt-4 border-t border-slate-100",
                                                ...
                                                div { class: "space-y-1.5",
                                                    div { class: "flex items-center justify-between text-xs font-black text-slate-500",
                                                        ...
                                                        span { class: "text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-100", "{item.count_7d}x salah" }
                                                    }
                                                ...
                                                div { class: "space-y-1.5",
                                                    div { class: "flex items-center justify-between text-xs font-black text-slate-500",
                                                        ...
                                                        span { class: "text-teal-600 bg-teal-50 px-2 py-0.5 rounded-lg border border-teal-100", "{item.count_30d}x salah" }
                                                    }
                                                    div { class: "w-full bg-slate-100 rounded-full h-2 overflow-hidden",
                    ...
                    if skill_points.is_empty() {
                        div { class: "bg-slate-50 border border-slate-200 border-dashed rounded-2xl p-12 text-center",
                            span { class: "text-4xl block mb-3", "📈" }
                            p { class: "text-slate-500 font-bold text-base", "Belum ada data tren keterampilan." }
                            p { class: "text-slate-400 text-xs mt-1", "Selesaikan materi pelajaran & kuis harian untuk melihat grafik tren keterampilan Anda." }
                        }
                    } else {
                        div { class: "space-y-6",
                            // Chart Wrapper
                            div { class: "border border-slate-200 bg-slate-50/50 p-4 sm:p-6 rounded-2xl shadow-inner",
                                svg {
