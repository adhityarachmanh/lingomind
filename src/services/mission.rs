...
pub async fn get_daily_mission_server(username: String, language: String) -> Result<DailyMission, ServerFnError> {
    #[cfg(not(target_arch = "wasm32"))]
    use sqlx::Row;

    let pool = super::db::get_pool();

    let due_row = sqlx::query("SELECT COUNT(*)::bigint AS cnt FROM flashcards WHERE username = $1 AND language = $2 AND due_at <= NOW()")
        .bind(&username)
        .bind(&language)
        .fetch_one(pool)
        .await
    ...
    let weak_row = sqlx::query("SELECT COUNT(*)::bigint AS cnt FROM weakness_logs WHERE username = $1 AND language = $2 AND created_at >= NOW() - INTERVAL '7 days'")
        .bind(&username)
        .bind(&language)
        .fetch_one(pool)
        .await
